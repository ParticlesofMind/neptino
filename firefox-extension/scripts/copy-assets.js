...
const rootIconsDir = join(rootDir, 'icons');
...
// Also mirror runtime files to root so loading firefox-extension/manifest.json works too.
const runtimeFiles = ['background.js', 'content-script.js', 'content-script.css', 'popup.js'];
runtimeFiles.forEach((fileName) => {
  const source = join(distDir, fileName);
...
if (!existsSync(rootIconsDir)) {
  mkdirSync(rootIconsDir, { recursive: true });
}

['icon16.png', 'icon48.png', 'icon128.png'].forEach((iconName) => {
  const source = join(iconsDir, iconName);
  const target = join(rootIconsDir, iconName);
  if (existsSync(source)) {
    copyFileSync(source, target);
  }
});
