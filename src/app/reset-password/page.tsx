...
function PasswordField({
  ...
  return (
    <div className="relative flex items-center rounded-xl border border-border bg-background focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15 transition-all duration-150">
      <span className="pl-3.5 text-muted-foreground/60 pointer-events-none flex-shrink-0">{LockIcon}</span>
      <input
        ...
        className="flex-1 min-w-0 px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 bg-transparent outline-none disabled:opacity-50"
      />
      <button
        type="button"
        onClick={() => setShow((s) => !s)}
        disabled={disabled}
        className="pr-3 pl-1 text-muted-foreground/60 hover:text-muted-foreground transition-colors duration-150 flex-shrink-0"
        tabIndex={-1}
