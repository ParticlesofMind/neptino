...
import { CARD_SPECS, SAMPLE_CONTENT, type CardGroup } from "@/components/coursebuilder/create/sidebar/make-panel-data"
...
const GROUP_LABELS: Record<CardGroup | "other", string> = {
  media: "Media",
  data: "Data",
  products: "Products",
...
const GROUP_DOT_CLASS: Record<CardGroup | "other", string> = {
  media: "bg-emerald-500", data: "bg-blue-500", products: "bg-amber-500",
  activities: "bg-violet-500", layout: "bg-slate-500", other: "bg-neutral-400",
}
  ...
  const cardItems = useMemo<GalleryItem[]>(() => {
    ...
    return allTypes.map((cardType) => {
      ...
      return {
        ...
        summary: spec?.description ?? "Card preview for visual regression testing.",
  ...
  return (
    <div className="min-h-[520px] h-[calc(100dvh-96px)] overflow-hidden">
      <div className="grid h-full min-h-0 gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
            ...
            {Object.entries(grouped).map(([group, items]) => (
              <div key={group} className="space-y-2">
                <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  <span className={`h-1.5 w-1.5 rounded-full ${GROUP_DOT_CLASS[group as CardGroup | "other"]}`} />
                  {GROUP_LABELS[group as CardGroup | "other"]}
                </p>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="overflow-hidden rounded-md border border-border/80 bg-background p-1">
                          {item.cardType.startsWith("layout-") ? (
                            ...
                            <div className="overflow-hidden" style={{ maxHeight: 140 }}>
                              <CardTypePreview cardType={item.cardType} content={item.content} />
                            </div>
                          )}
                        </div>
                      </div>
        ...
        >
          <div
            className={isSingleLayoutPreview
          ...
          >
              {viewMode === "single" ? (
                isSingleLayoutPreview ? (
                  ...
                  <div
                    className="mx-auto rounded-xl border border-border bg-background p-4 shadow-sm"
                    style={{ width: PREVIEW_FRAME.width, minHeight: PREVIEW_FRAME.height }}
                  >
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {gridItems.map((item) => (
                    <article key={item.cardType} className="rounded-xl border border-border bg-background p-3 shadow-sm">
                      ...
                      <div
                        className="rounded-lg border border-border/80 bg-muted/10 p-2"
                        style={{ minHeight: item.cardType.startsWith("layout-") ? 640 : PREVIEW_FRAME.height }}
                      >
                        {item.cardType.startsWith("layout-") ? (
                          <LayoutSpecThumbnail layoutType={item.cardType} />
                        ) : (
