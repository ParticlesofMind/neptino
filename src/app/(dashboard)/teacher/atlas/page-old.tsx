...
export default async function TeacherAtlasPage({
  ...
  return (
    <section className="space-y-6">
      ...
      <form className="rounded-2xl border-2 border-primary/20 bg-gradient-to-br from-background to-accent/10 p-5 shadow-lg" method="get">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          <input
            ...
            className="min-w-[280px] rounded-lg border border-border/30 bg-background px-3 py-2 text-sm"
          />

          <select
            name="domain"
            defaultValue={selectedDomain ?? "all"}
            className="min-w-[170px] rounded-lg border border-border/30 bg-background px-3 py-2 text-sm"
          >
      ...
      <div className="flex items-center justify-between gap-3 rounded-xl bg-accent/20 px-3 py-2 text-xs text-muted-foreground">
        <span>
      ...
      {panelItem && (
        <section className="mt-4 rounded-2xl border border-border/40 bg-background/80 p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-foreground">{panelItem.title}</h2>
          ...
          {panelMediaByType.length > 0 && (
            <div className="mt-4 space-y-3">
              {panelMediaByType.map((group) => (
                <div key={group.mediaType} className="rounded-xl bg-accent/30 p-3">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    {group.mediaType}
                  </h3>
                  <div className="mt-2 space-y-2">
                    {group.resources.map((resource) => (
                      <div key={resource.id} className="rounded-lg border border-border/30 bg-background/80 p-2.5">
          ...
          {panelItem.metadata && Object.keys(panelItem.metadata).length > 0 && (
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {Object.entries(panelItem.metadata)
                ...
                .map(([key, value]) => (
                  <div key={key} className="rounded-lg border border-border/30 bg-background/80 p-2.5">
                    <p className="text-[11px] uppercase tracking-wide text-muted-foreground">{key}</p>
                    <p className="mt-1 text-xs text-foreground break-words">{formatMetadataValue(value)}</p>
                  </div>
                ))}
            </div>
          )}
      ...
      <div className={isLarge ? "mt-4 grid gap-5" : "mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"}>
        {mediaFilterActive
          ? filteredMediaCards.map(({ item, resource }, index) => {
            ...
            return (
              <article
                key={resource.id}
                className={isLarge
                  ? "rounded-2xl border border-border/40 bg-background/80 p-6 shadow-sm"
                  : "rounded-2xl border border-border/40 bg-background/80 p-4 shadow-sm"
                }
              >
          ...
          if (isLarge) {
            return (
              <article key={item.id} className="rounded-2xl border border-border/40 bg-background/80 p-6 shadow-sm">
                <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full bg-accent/60 px-2 py-0.5 text-xs text-muted-foreground">{item.knowledge_type}</span>
                      {item.era_label && (
                        <span className="rounded-full bg-accent/60 px-2 py-0.5 text-xs text-muted-foreground">{item.era_label}</span>
                      )}
                      {item.depth && (
                        <span className="rounded-full bg-accent/60 px-2 py-0.5 text-xs text-muted-foreground">{capitalize(item.depth)}</span>
                      )}
                      <span className="rounded-full bg-accent/60 px-2 py-0.5 text-xs text-muted-foreground">Card #{index + 1}</span>
                    </div>

                    <h2 className="mt-3 text-2xl font-bold text-foreground leading-tight">{item.title}</h2>
                    ...
                    {domainBadges.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {domainBadges.map((domain) => (
                          <span key={`${item.id}-${domain}`} className="rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary">
                            {domain}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="rounded-xl border border-border/30 bg-background/70 p-3">
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Related</p>
                        {item.tags && item.tags.length > 0 ? (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {item.tags.slice(0, 7).map((tag) => (
                              <Link
                                ...
                                className="rounded-full border border-border/40 px-2 py-0.5 text-[11px] text-muted-foreground hover:bg-accent/40 hover:text-foreground"
                              >
                                {tag}
                              </Link>
                            ))}
                          ...
                          <p className="mt-2 text-sm text-muted-foreground italic">No related items available.</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          return (
            <article key={item.id} className="rounded-2xl border border-border/40 bg-background/80 p-4 shadow-sm">
              {previewMedia && (
                <div className="mb-3 w-full overflow-hidden rounded-lg border border-border/30 bg-accent/20">
                  <img
                    src={previewMedia.url}
                    alt={previewMedia.title || `${item.title} image`}
              ...
              {!previewMedia && hasCompendium && (
                <div className="mb-3 w-full aspect-[4/3] overflow-hidden rounded-lg border border-border/30 bg-accent/20">
                  <div className="flex h-full w-full items-center justify-center gap-2 text-muted-foreground">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full bg-accent/60 px-2 py-0.5 text-xs text-muted-foreground">
                  {item.knowledge_type}
                </span>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">
            ...
            <Link
              ...
              className={
                pageNumber === activePage
                  ...
                  : "rounded-md border border-border/30 px-3 py-1.5 text-xs hover:bg-accent"
              }
            >
              {pageNumber}
            </Link>
