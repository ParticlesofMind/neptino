...
export function bindSnapMenu(): void {
  ...
  // Handle reference mode actions (Canvas, Object, Grid)
  snapMenu.querySelectorAll<HTMLElement>('[data-reference-mode]').forEach(item => {
    item.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();

      const referenceMode = item.getAttribute('data-reference-mode');
      
      // Remove active class from all reference mode items
      snapMenu.querySelectorAll('[data-reference-mode]').forEach(refItem => {
        refItem.classList.remove('engine__snap-item--active');
      });
      
      // Add active class to clicked item
      item.classList.add('engine__snap-item--active');
      
      // Save reference mode to localStorage
      try {
        localStorage.setItem('referenceMode', referenceMode || 'canvas');
      } catch (error) {
        console.warn('Failed to save reference mode:', error);
      }
      
      // Show/hide grid options based on selection
      ...
      try {
        (snapManager as any).setPrefs?.({ referenceMode: referenceMode });
      } catch (error) {
        console.warn('Failed to set reference mode:', error);
      }

      console.log('Smart guide reference mode selected:', referenceMode);
    });
  });

  // Handle grid options
  const showGridToggle = snapMenu.querySelector('#show-grid') as HTMLInputElement | null;
  const gridSpacingInput = snapMenu.querySelector('#grid-spacing') as HTMLInputElement | null;

  if (showGridToggle) {
    // Load saved grid visibility state
    try {
      ...
      console.warn('Failed to load show grid state:', error);
    }
    ...
    showGridToggle.addEventListener('change', (e) => {
      ...
      try {
        ...
        console.warn('Failed to save show grid state:', error);
      }

      // Update snap manager preferences
      try {
        (snapManager as any).setPrefs?.({ showGrid: showGrid });
      } catch (error) {
        console.warn('Failed to set show grid preference:', error);
      }

      console.log('Show grid toggled:', showGrid);
    });
  }

  if (gridSpacingInput) {
    // Load saved grid spacing
    try {
      ...
      console.warn('Failed to load grid spacing:', error);
    }

    gridSpacingInput.addEventListener('input', (e) => {
      e.stopPropagation();
      const spacing = Math.max(1, Math.min(100, parseInt(gridSpacingInput.value) || 20));
      gridSpacingInput.value = String(spacing); // Clamp the value

      // Save state to localStorage
      try {
        localStorage.setItem('gridSpacing', String(spacing));
      } catch (error) {
        console.warn('Failed to save grid spacing:', error);
      }

      // Update snap manager preferences
      try {
        (snapManager as any).setPrefs?.({ gridSpacing: spacing });
      } catch (error) {
        console.warn('Failed to set grid spacing preference:', error);
      }

      console.log('Grid spacing changed:', spacing);
    });
  }
  ...
  try {
    ...
    // Load saved reference mode
    try {
      const savedReferenceMode = localStorage.getItem('referenceMode') || 'canvas';
      snapMenu.querySelectorAll('[data-reference-mode]').forEach(item => {
        item.classList.remove('engine__snap-item--active');
        if (item.getAttribute('data-reference-mode') === savedReferenceMode) {
          item.classList.add('engine__snap-item--active');
        }
      });
      ...
      // Update snap manager with loaded reference mode
      (snapManager as any).setPrefs?.({ referenceMode: savedReferenceMode });
    } catch (error) {
