...
export class ColorSelector {
  ...
  private createColorButton(): void {
    ...
    button.className = 'button button--color-selector';
    button.type = 'button';
    ...
    button.innerHTML = `
      <div class="button--color-selector__preview" style="background-color: ${this.currentColor.hex}"></div>
      <span class="button--color-selector__name">${this.currentColor.name}</span>
      <svg class="button--color-selector__arrow" width="12" height="12" viewBox="0 0 12 12" fill="currentColor">
  ...
  private createDropdown(): void {
    ...
    this.dropdown.className = 'button--color-selector__dropdown';
    this.dropdown.setAttribute('role', 'listbox');
    this.dropdown.setAttribute('aria-label', 'Color options');

    this.colors.forEach((color) => {
      const option = document.createElement('button');
      option.className = 'button--color-selector__option';
      option.type = 'button';
      ...
      option.innerHTML = `
        <div class="button--color-selector__swatch" style="background-color: ${color.hex}"></div>
        <span class="button--color-selector__label">${color.name}</span>
        ${color.hex === this.currentColor.hex ? '<svg class="button--color-selector__check" width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M13.485 3.495a.75.75 0 011.06 1.06l-7 7a.75.75 0 01-1.06 0l-3.5-3.5a.75.75 0 111.06-1.06L6.97 9.44l6.515-5.945z"/></svg>' : ''}
      `;
  ...
  private handleDropdownKeydown(e: KeyboardEvent): void {
    const options = Array.from(this.dropdown?.querySelectorAll('.button--color-selector__option') || []) as HTMLButtonElement[];
    ...
    switch (e.key) {
      ...
      case 'Escape':
        this.closeDropdown();
        (this.container.querySelector('.button--color-selector') as HTMLButtonElement).focus();
        break;
    }
  }
  ...
  private updateButton(): void {
    const button = this.container.querySelector('.button--color-selector') as HTMLButtonElement;
    const preview = button.querySelector('.button--color-selector__preview') as HTMLElement;
    const name = button.querySelector('.button--color-selector__name') as HTMLElement;
  ...
  private updateDropdownSelection(): void {
    ...
    const options = this.dropdown.querySelectorAll('.button--color-selector__option');
